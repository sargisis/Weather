...
void AuthWindow::handleGoogleSignIn() {
    ...
    // ID клиента и URL перенаправления для Google OAuth.
    // Эти значения должны соответствовать вашим настройкам в Google Developer Console.
    const QString clientId = "61085252023-kgrp7cksj5vqs6bet9q3ldfaot81nraf.apps.googleusercontent.com";
    const QString redirectUri = "http://localhost:8080"; // Локальный адрес, на который Google будет перенаправлять.

    // Формируем URL для авторизации Google.
    // response_type=code указывает, что мы ожидаем код авторизации.
    // scope определяет, к каким данным мы запрашиваем доступ (openid, email, profile).
    // access_type=offline запрашивает refresh_token для долгосрочного доступа.
    ...
    // Открываем URL в браузере по умолчанию. Пользователь будет аутентифицироваться через Google.
...
void AuthWindow::exchangeCodeForToken(const QString& code) {
    // ID клиента, секрет клиента и URL перенаправления.
    // clientSecret является конфиденциальным и в реальных приложениях должен храниться безопасно.
    const QString clientId = "61085252023-kgrp7cksj5vqs6bet9q3ldfaot81nraf.apps.googleusercontent.com";
    const QString clientSecret = "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";
    const QString redirectUri = "http://localhost:8080";

    QUrl tokenUrl("https://oauth2.googleapis.com/token"); // URL конечной точки для обмена токенов.
    QNetworkRequest request(tokenUrl);
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/x-www-form-urlencoded"); // Устанавливаем тип контента.

    QUrlQuery params; // Создаем объект для параметров запроса.
    params.addQueryItem("code", code); // Код авторизации, полученный от Google.
    params.addQueryItem("client_id", clientId);
    params.addQueryItem("client_secret", clientSecret);
    params.addQueryItem("redirect_uri", redirectUri);
    params.addQueryItem("grant_type", "authorization_code"); // Указываем тип гранта.

    // Соединяем сигнал finished от сетевого менеджера с лямбда-функцией для обработки ответа.
    connect(network, &QNetworkAccessManager::finished, this, [this](QNetworkReply* reply) {
        if (reply->error() == QNetworkReply::NoError) { // Если запрос завершен без ошибок
            QByteArray replyData = reply->readAll(); // Читаем данные ответа.
            QJsonDocument doc = QJsonDocument::fromJson(replyData); // Парсим JSON-ответ.
            QString refreshToken = doc["refresh_token"].toString(); // Извлекаем refresh_token.

            // Сохраняем refresh_token в настройках приложения для автоматического входа в будущем.
            QSettings settings("MyCompany", "WeatherApp");
            settings.setValue("refresh_token", refreshToken);

            QMessageBox::information(this, "Google Login", "Login successful!"); // Показываем сообщение об успехе.
            emit authSuccess(); // Испускаем сигнал об успешной аутентификации.
            this->close();     // Закрываем окно аутентификации.
        } else {
            // Если произошла ошибка при обмене токенов, показываем предупреждение.
            QMessageBox::warning(this, "Google Login Error", reply->errorString());
        }
        reply->deleteLater(); // Удаляем объект QNetworkReply.
    });

    // Отправляем POST-запрос на сервер Google для обмена кода на токены.
